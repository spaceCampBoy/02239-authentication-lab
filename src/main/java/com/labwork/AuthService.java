...
public class AuthService implements AuthServiceInterface {
	...
	private ArrayList<String> returnUserOperations(String username) {
		...
		userOperations = getRoleOperations(username);
	...
	private ArrayList<String> getRoleOperations(String role) {
		...
		try {
			File myObj = new File("./src/main/java/com/labwork/accesslist.txt");
			//String myObj = new String(Files.readAllBytes(Paths.get("./src/main/java/com/labwork/acl-policy.json")));
			Scanner myReader = new Scanner(myObj);
			while (myReader.hasNextLine()) {
				String data = myReader.nextLine();
				String[] lineSplit = data.split(":");
